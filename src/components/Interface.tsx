...
import { Maximize, Sun, Layers, Palette, RefreshCw, Image } from 'lucide-react';
...
const Interface = () => {
  ...
  const patternItems = patterns.map(({ id, val, src }) => (
    <li key={id} className="cursor-pointer hover:scale-105">
      <button value={val} onClick={(e) => dispatch(setCurPiece(e.currentTarget.value))}>
        <img src={src} alt={val} />
      </button>
    </li>
  ));
  ...
  const fileSelected = async (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      dispatch(setCurrentTexture(await toBase64(e.target.files[0])));
    }
  };
  ...
  return (
    <NavigationMenu>
      <NavigationMenuList>
        <NavigationMenuItem onClick={selectFile}>
          <NavigationMenuTrigger>
            <Image />
            <input
              type="file"
          ...
          <NavigationMenuContent onClick={selectFile}>
            <div className="grid grid-cols-2 gap-3 p-6 md:w-[400px]">Select picture file</div>
          </NavigationMenuContent>
        </NavigationMenuItem>
        <NavigationMenuItem>
          <NavigationMenuTrigger>
          ...
          <NavigationMenuContent>
            <ul className="grid grid-cols-2 gap-3 p-6 md:w-[400px]">{patternItems}</ul>
          </NavigationMenuContent>
        </NavigationMenuItem>
        ...
        <NavigationMenuItem>
          ...
          <NavigationMenuContent>
            <ul className="grid grid-cols-4 gap-3 p-6 md:w-[400px]">{scaleItems}</ul>
          </NavigationMenuContent>
        </NavigationMenuItem>
