...
const initScene = (canvas: HTMLCanvasElement, bgImage: HTMLImageElement) => {
  ...
  function checkTarget(event: MouseEvent) {
    ...
    const isObj = intersects.find((intersect) => intersect.object.userData.name === 'object');
    if (isObj) {
  ...
  bgImage.width = canvas.offsetWidth;
  bgImage.height = canvas.offsetHeight;
