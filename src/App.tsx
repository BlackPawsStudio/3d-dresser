...
export const App = () => {
  ...
  const textureLoader = useMemo(() => new THREE.TextureLoader(manager), [manager]);

  const alphaTexture = useMemo(() => {
    const alphaTexture = textureLoader.load(
      new URL(`./assets/${curPiece}/${curPiece}_overlay.png`, import.meta.url).href
    );
  ...
  const lightTexture = useMemo(() => {
    const lightTexture = textureLoader.load(
      new URL(`./assets/${curPiece}/${curPiece}_light.png`, import.meta.url).href
    );
    lightTexture.encoding = THREE.sRGBEncoding;
    return lightTexture;
  }, [curPiece, textureLoader]);

  const [pattern, setPattern] = useState<THREE.Texture>(() => {
    const newPattern = textureLoader.load(new URL(currentTexture, import.meta.url).href);
    newPattern.encoding = THREE.sRGBEncoding;
    newPattern.center.set(0.5, 0.5);
    newPattern.wrapS = newPattern.wrapT = THREE.RepeatWrapping;
    return newPattern
  });

  useEffect(() => {
    const newPattern = textureLoader.load(currentTexture);
    newPattern.encoding = THREE.sRGBEncoding;
    newPattern.center.set(0.5, 0.5);
    newPattern.wrapS = newPattern.wrapT = THREE.RepeatWrapping;
  ...
  I couldn't find another way to dynamically change it with three js. it has setHSL 
  ...
  js scene camera parameters a little and sizes of three js planes that 
  ...
  const [shirtMaterial, setShirtMaterial] = useState<THREE.MeshStandardMaterial | null>(null);
  ...
  useEffect(() => {
    if (canvasRef.current && bgImageRef.current) {
      ...
      const scene = initScene(canvasRef.current, bgImageRef.current);
      objLoader.load(
        new URL(`./assets/${curPiece}/${curPiece}_3d_model.obj`, import.meta.url).href,
        (object) => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <canvas
        ...
        className={cn(
          'mt-5 w-[90vh] mx-auto aspect-square sm:h-auto',
      ...
      <div className="absolute z-[2] aspect-square mt-[60px] w-[90vh] sm:h-auto pointer-events-none">
        <img
          ref={bgImageRef}
          alt="pattern"
          className={cn('pointer-events-none mx-auto', isCanvasLoading && 'opacity-0')}
        />
      </div>
    </div>
  );
